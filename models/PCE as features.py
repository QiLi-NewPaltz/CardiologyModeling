...
xy = pd.concat([targets,features_bl[['Carrier_General Managed Care', 'Carrier_Humana', 'Carrier_Medicaid',
       'Carrier_Medicaid Managed Care', 'Carrier_Medicare',
       'Carrier_Medicare Managed Care', 'Carrier_SelfPay',
       'Carrier_United Healthcare', 'Carrier_Worker\'s Comp']]], axis=1)
xy.shape

xy_a = pd.concat([target_a,features_a_bl[['Carrier_General Managed Care', 'Carrier_Humana', 'Carrier_Medicaid',
       'Carrier_Medicaid Managed Care', 'Carrier_Medicare',
       'Carrier_Medicare Managed Care', 'Carrier_SelfPay',
       'Carrier_United Healthcare', 'Carrier_Worker\'s Comp']]], axis=1)
...
X_train, X_test, y_cad_cvd_train, y_cad_cvd_test = train_test_split(X, y, random_state=66,test_size=0.1)
X_bl_train, X_bl_test, y_bl_cad_cvd_train, y_bl_cad_cvd_test = train_test_split(X_bl, y, random_state=66,test_size=0.1)
...
#plt.show()
plt.savefig("data/figure_datamain.jpg", dpi=1200)
