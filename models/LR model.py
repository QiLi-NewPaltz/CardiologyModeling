...
for train_index, test_index in kf.split(X):
    ...
    lr_cad_cvd= LogisticRegression(random_state=0).fit(X_train, Y_train)
    y_pred_lr_cad_cvd = lr_cad_cvd.predict(X_test)
    y_pred_lr_cad_cvd_p = lr_cad_cvd.predict_proba(X_test)[:,1]
    print (str(k)+"\t"+"lr_lv\t"+result_prob(Y_test,y_pred_lr_cad_cvd_p)+"\t"+result(Y_test,y_pred_lr_cad_cvd))
    
    lr_a_prediction_proba = lr_cad_cvd.predict_proba(X_a_test)[:, 1]
    lr_a_prediction = lr_cad_cvd.predict(X_a_test)
    print (str(k)+"\t"+"lr_lv_a\t"+result_prob(Y_a_test,lr_a_prediction_proba)+"\t"+result(Y_a_test,lr_a_prediction))

    
    lr_bl_cad_cvd= LogisticRegression(random_state=0).fit(X_bl_train, Y_train)
    y_pred_lr_bl_cad_cvd = lr_bl_cad_cvd.predict(X_bl_test)
    y_pred_lr_bl_cad_cvd_p = lr_bl_cad_cvd.predict_proba(X_bl_test)[:,1]
    print (str(k)+"\t"+"lr_bl\t"+result_prob(Y_test,y_pred_lr_bl_cad_cvd_p)+"\t"+result(Y_test,y_pred_lr_bl_cad_cvd))
    
    lr_bl_a_prediction_proba = lr_bl_cad_cvd.predict_proba(X_bl_a_test)[:, 1]
    lr_bl_a_prediction = lr_bl_cad_cvd.predict(X_bl_a_test)
    print (str(k)+"\t"+"lr_bl_a\t"+result_prob(Y_a_test,lr_bl_a_prediction_proba)+"\t"+result(Y_a_test,lr_bl_a_prediction))
