...
for train_index, test_index in kf.split(X):
    ...
    #print (str(train_index.size)+"\t"+str(test_index.size))
    ...
    print (str(k)+"\t"+"mlp_lv\t"+result_prob(Y_test,mlp_prediction_proba)+"\t"+result(Y_test,mlp_prediction))
    
    mlp_a_prediction_proba = mlp_clf.predict_proba(X_a_test)[:, 1]
    mlp_a_prediction = mlp_clf.predict(X_a_test)
    print (str(k)+"\t"+"mlp_lv_a\t"+result_prob(Y_a_test,mlp_a_prediction_proba)+"\t"+result(Y_a_test,mlp_a_prediction))

    
    # Multi-layer Perceptron Classifier
    mlp_bl_clf = MLPClassifier(alpha=1, hidden_layer_sizes=150)
    mlp_bl_clf.fit(X_bl_train, Y_train)
    mlp_bl_prediction_proba = mlp_bl_clf.predict_proba(X_bl_test)[:, 1]
    mlp_bl_prediction = mlp_bl_clf.predict(X_bl_test)
    print (str(k)+"\t"+"mlp_bl\t"+result_prob(Y_test,mlp_bl_prediction_proba)+"\t"+result(Y_test,mlp_bl_prediction))
    
    mlp_bl_a_prediction_proba = mlp_bl_clf.predict_proba(X_bl_a_test)[:, 1]
    mlp_bl_a_prediction = mlp_bl_clf.predict(X_bl_a_test)
    print (str(k)+"\t"+"mlp_bl_a\t"+result_prob(Y_a_test,mlp_bl_a_prediction_proba)+"\t"+result(Y_a_test,mlp_bl_a_prediction))
