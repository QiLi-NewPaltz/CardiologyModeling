...
for train_index, test_index in kf.split(X):
    ...
    ascvd_prediction_proba = feature_a.loc[test_a_test]['ASCVD_10_YR_SCORE']
    ylabel = Y_test.loc[test_a_test]
    
    for i in range (1, 21, 1):
        ascvd_prediction = (ascvd_prediction_proba >= i/).astype(bool) # set threshold as 0.1
        print (str(k)+"\t"+str(i)+"\t"+result_prob(ylabel, ascvd_prediction_proba)+"\t"
               +result(ylabel, ascvd_prediction))
